fl.trace("");
fl.trace("Revan - Rename selected library items - CAPS_LIKE_THIS");
...
function rename(item)
{
	...
	var newName = toCaps(oldName); 
	fl.trace("old name:" + oldName + ", new name:" + newName);
	library.renameItem(newName);
}

function toCaps(str) 
{
	result = str.replace(/([A-Z])/g, '_$1');
